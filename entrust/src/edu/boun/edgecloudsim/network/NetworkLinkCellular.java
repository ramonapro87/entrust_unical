...
public abstract class NetworkLinkCellular extends NetworkLink {
	...
	protected NetworkLinkCellular(Host src, Host dst, SimManager simulationManager, NetworkLinkTypes type) {
		super(src, dst, simulationManager, type);
		setBandwidth(SimSettings.getInstance().getGsmBandwidth());
		setDelay(SimSettings.getInstance().getGsmPropagationDelay()); 
	}
