 ...
 * A WiFi network link.
 */
public class NetworkLinkWifiDeviceToDevice extends NetworkLinkWifi {

	public NetworkLinkWifiDeviceToDevice(Host src, Host dst, SimManager simulationManager,
			NetworkLinkTypes type) {
		super(src, dst, simulationManager, type);

		setEnergyModel(new DefaultEnergyNetworkModel(SimSettings.getInstance().getWifiDeviceTransmissionWattHourPerBit()
				+ SimSettings.getInstance().getWifiDeviceReceptionWattHourPerBit(), this));
	}
