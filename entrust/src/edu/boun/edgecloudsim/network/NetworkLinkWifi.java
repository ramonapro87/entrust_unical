...
public abstract class NetworkLinkWifi extends NetworkLink {
	...
	protected NetworkLinkWifi(Host src, Host dst, SimManager simulationManager, NetworkLinkTypes type) {
        super(src, dst, simulationManager, type);
        setBandwidth(SimSettings.getInstance().getWlanBandwidth());
        setDelay(SimSettings.getInstance().getInternalLanDelay()); 
    }
